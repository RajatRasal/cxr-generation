...
class ImageEncoder(nn.Module):
    ...
    def reload_encoder_with_dilation(
        ...
        """Workaround for enabling dilated convolutions after model
        initialization.
...
class MultiImageEncoder(ImageEncoder):
    """Multi-image encoder trunk module for the ``ImageModel`` class. It can be
    used to encode multiple images into combined latent representation.
    Currently it only supports two input images but can be extended to support
    more in future.
...
def get_encoder_output_dim(module: torch.nn.Module, device: torch.device) -> int:
    """Calculate the output dimension of an encoder by making a single forward
    pass.
