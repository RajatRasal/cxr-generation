...
import pytest
import torch
from health_multimodal.image import (ImageEncoderType, ImageInferenceEngine,
                                     ImageModel)
...
def _get_vlp_inference_engine() -> ImageTextInferenceEngine:
    image_inference = ImageInferenceEngine(
        image_model=ImageModel(
            img_encoder_type=ImageEncoderType.RESNET50.value,
            joint_feature_size=JOINT_FEATURE_SIZE,
        ),
        transform=create_chest_xray_transform_for_inference(
            resize=512, center_crop_size=CENTER_CROP_SIZE
        ),
    )
...
def test_vlp_inference(height: int, query_text: Union[str, List[str]]) -> None:
    ...
    with tempfile.NamedTemporaryFile(suffix=".jpg") as f:
        ...
        image_embedding, size = image_inference.get_projected_patch_embeddings(
            image_path
        )

        assert (width, height) == size
        expected_image_embedding_size = image_embedding_shapes[CENTER_CROP_SIZE]
        assert image_embedding.shape == (
            *expected_image_embedding_size,
            JOINT_FEATURE_SIZE,
        )
        normalized_image_embedding = torch.norm(image_embedding, p=2, dim=-1)
        assert torch.allclose(
            normalized_image_embedding, torch.ones_like(normalized_image_embedding)
        )

        text_embedding = (
            img_txt_inference.text_inference_engine.get_embeddings_from_prompt(
                query_text
            )
        )
        assert text_embedding.shape == (1, JOINT_FEATURE_SIZE)

        similarity_map = img_txt_inference._get_similarity_map_from_embeddings(
...
@pytest.mark.parametrize(
    "query_text", ("this is a test", ["Test prompt 1", "Test prompt 2"])
)
def test_vlp_inference_global_similarity(query_text: str) -> None:
    ...
    with tempfile.NamedTemporaryFile(suffix=".jpg") as f:
        ...
        sim_score = img_txt_inference.get_similarity_score_from_raw_data(
            image_path=image_path, query_text=query_text
        )
